...
if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id'])) {
   ...
   else
   {
      ...
      mysqli_query($conn, "UPDATE recommenders SET rec_name='$rec_name', rec_email='$rec_email' WHERE id='$id'") or die(mysqli_connect_error());
...
// if the form hasn't been submitted, get the data from the db and display the form
    if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id']) && $_REQUEST['id'] > 0) {
      // query db
   $id = check_input($conn, $_REQUEST['id']);
   $result = mysqli_query($conn, "SELECT * FROM recommenders WHERE id=$id") or die(mysqli_connect_error());
