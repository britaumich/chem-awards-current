...
if (isset($_REQUEST['dataid']) && is_numeric($_REQUEST['dataid'])) {
   ...
   if ($status == 'error') {
    ...
    // if either field is blank, display the form again
      renderForm($conn, $dataid, $name, $year, $status, $comment, $error);
   }  // empty
   else
   {
       // save the data to the database
      $sql1 =  "UPDATE faculty_awards_notchem SET name = '$name', status='$status', year='$year', comment='$comment'  WHERE id='$dataid'";
      mysqli_query($conn, "UPDATE faculty_awards_notchem SET name = '$name', status='$status', year='$year', comment='$comment'  WHERE id='$dataid'") or die(mysqli_error($conn));
   ...
   $row = mysqli_fetch_array($result, MYSQLI_BOTH);
    if($row) {
         // get data from db
          ...
          $comment = $row['comment'];
       // show form
      renderForm($conn, $dataid, $name, $year, $statusid, $comment, $error);
